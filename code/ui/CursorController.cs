	...
	public class CursorController : Panel
	{
		...
		private void BuildInput( InputBuilder builder )
		{
			...
			if ( builder.Released( InputButton.Attack2 ) && !ItemManager.Instance.Ghost.IsValid() )
			{
				...
				{
					var trace = Trace.Ray( builder.Position, builder.Position + builder.CursorAim * 10000f )
						.Radius( 5f )
						.Run();
			...
			else if ( IsSelecting )
			{
				...
				{
					var trace = Trace.Ray( builder.Position, builder.Position + builder.CursorAim * 10000f ).EntitiesOnly().Run();
